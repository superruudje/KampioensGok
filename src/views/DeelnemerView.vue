                    ...
                    <div class="card border-0 rounded-0 shadow-sm h-100">
                        <div
                            ...
                            <span class="txt-orange fw-bold" style="font-size: 5rem">
                                <number-counter
                                    :number="tournament.getParticipantTotalScore(participant.team_name)"></number-counter>
                            </span>
                        ...
                        <div class="card-body position-relative p-4">
                            ...
                            <template v-else>
                                <match-day-prediction v-for="(matches, match_day) in matches_played_poule"
                                                      :match_day="match_day" :matches="matches" :name="participant.team_name"
                                    ...
                                    <div class="d-flex align-items-center justify-content-between">
                                        <span class="txt-blue">
                                            ...
                                            <span v-if="q.type === 'exact'"
                                                  class="small fst-italic"> ({{ getPercentage(data[idx].find(i => i.id === participant.bonus[idx])?.count) }}% denkt dit ook{{
                                                    q.now ? `, nu ${q.now}` : ""
                                                }})</span>
                                        ...
                                        <template v-if="bonus[idx]">
                                            <span v-if="q.type === 'exact'"
                                                  class="badge bg-orange">+{{ participant.bonus[idx] === bonus[idx] ? q.p : 0 }}</span>
...
const series = computed(() => {
    ...
    Object.keys(matches_played_by_day.value).forEach((key, idx) => {
        const score = tournament.getParticipantTotalScore(participant.value.team_name, idx + 1)
